...
export default function Dashboard({ apps, onCardClick }: Props) {
  const stats = useMemo(() => {
    ...
    const processedApps = apps.filter(
      (a) => a.received_date && (a.status === 'sent_to_approval' || a.status === 'approved' || a.status === 'rejected')
    );
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-6 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <KpiCard label="Sent / Approved" value={stats.approved} icon={<CheckCircle2 size={18} />} tone="from-emerald-500 to-green-600" onClick={onCardClick ? () => onCardClick('approved') : undefined} />
